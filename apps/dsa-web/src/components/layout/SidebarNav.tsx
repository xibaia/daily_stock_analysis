...
import { BarChart3, Bell, BriefcaseBusiness, Home, LogOut, MessageSquareQuote, RadioTower, Settings2 } from 'lucide-react';
...
const NAV_ITEMS: NavItem[] = [
  ...
  { key: 'deepear', label: 'DeepEar', to: '/deepear', icon: RadioTower },
  { key: 'settings', label: '设置', to: '/settings', icon: Settings2 },
];

export const SidebarNav: React.FC<SidebarNavProps> = ({ collapsed = false, onNavigate }) => {
  const { authEnabled, logout, role } = useAuth();
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <nav className="flex flex-1 flex-col gap-1.5" aria-label="主导航">
        {visibleNavItems.map(({ key, label, to, icon: Icon, exact, badge }) => (
          <NavLink
            key={key}
            to={to}
            end={exact}
