...
| `ADMIN_AUTH_ENABLED` | Web 登录：设为 `true` 启用密码保护；首次访问在网页设置管理员密码，可在「系统设置 > 修改密码」修改；忘记管理员密码执行 `python -m src.auth reset_password`。如需只读/访客登录，可在服务器执行 `python -m src.auth set_user_password` 设置访客密码；访客不能进入系统设置或执行 `.env` 备份导入导出。角色化 session 会使旧版本登录 Cookie 失效，重新登录即可。Web 的 `.env` 备份导入导出仅在开启该开关并使用管理员会话后可用（桌面端不受此限制）。 | `false` |
