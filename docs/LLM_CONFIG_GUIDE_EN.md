...
- For YAML routes that explicitly require an Anthropic top-level `thinking` parameter, configure the payload under that deployment's `model_info.dsa_thinking` (see `docs/examples/litellm_config.example.yaml`). Regular analysis and Agent calls promote it consistently and add it to LiteLLM's allowlist; OpenAI-compatible routes keep `extra_body` semantics. DSA never infers this capability from names such as `kimi`.
