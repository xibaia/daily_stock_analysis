...
The Compose setup uses the host `.env` as the base startup configuration and an optional `data/runtime.env` as a later override layer. Compose sets `ENV_FILE=/app/data/runtime.env`, so WebUI writes go to the persistent `data` volume. The `docker run` examples still use `--env-file` for startup injection. Neither approach mounts the host `.env` as a single file inside the container. When the active `.env` file does not contain a key, the WebUI Settings page falls back to showing the same key from startup-injected process environment variables. The raw `.env` export still contains only the active config file content.

With Compose, WebUI-saved runtime config is written to `data/runtime.env` and survives container deletion, recreation, and upgrades; that file is loaded as the second `env_file`, so its values take precedence over same-name base values in the host `.env`. It may be absent on first deployment and will be created on the first save. For `docker run` or other manual startup methods, point `ENV_FILE` at a writable data-volume file and remove or update same-name startup environment overrides, so they do not take precedence over the runtime file. Do not use a single-file `.env` bind mount.
